package com.commnunication.blog.config;
...
import com.commnunication.blog.service.AppAuthProvider;
...
*/
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().exceptionHandling().authenticationEntryPoint(new Http403ForbiddenEntryPoint() {
		}).and().authenticationProvider(getProvider()).formLogin().loginProcessingUrl("/login")
				...
				.antMatchers("/user").permitAll().antMatchers("/posts","/comments").authenticated().
